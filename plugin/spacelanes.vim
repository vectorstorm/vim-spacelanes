...
function! s:Spacelane_GoTo(label) "{{{
	if ( has_key( s:spacelane_destinations, a:label ) )
		if ( bufname('') == s:spacelane_destinations[a:label] )
			echom "Already at" s:spacelane_destinations[a:label]
		else
			execute "e " s:spacelane_destinations[a:label]
			return 0
		endif
	else
...
function! s:Spacelane_SetTo(label) "{{{
	...
	let s:spacelane_destinations[a:label] = bufname('')
	echom a:label " : " s:spacelane_destinations[a:label]
	return 0
endfunction "}}}

function! s:Spacelane_List() "{{{
	return 0
endfunction "}}}
